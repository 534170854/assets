...
class AssetInventory(models.Model):
    ...
    def action_done(self):
        ...
        for inventory_line_id in self.inventory_line_ids:
            ...
            if inventory_line_id.inventory_result == "profit":
                vals = {
                    ...
                    "category_id": inventory_line_id.category_id.id,
                    "value": inventory_line_id.value,
                    "value_residual": inventory_line_id.value,
                    "company_id": inventory_line_id.inventory_id.company_id.id,
                    "currency_id": inventory_line_id.inventory_id.company_id.currency_id.id,
                    "date": date_done,
                    "state": "draft",
                }
                ...
                move_line_ids.append((0, 0, {
                    ...
                    "date": self.date_done,
                    "dest_owner_employee_id": self.owner_employee_id.id,
                    "dest_owner_department_id": self.owner_department_id.id,
                    "dest_employee_id": self.employee_id.id,
                    "dest_department_id": self.department_id.id,
        ...
        self.write(val)
        self.mapped('move_line_ids').action_done()
        return True
...
class AssetInventoryLine(models.Model):
    ...
    inventory_result = fields.Selection(
        [("draft", "Draft"), ("profit", "Profit"), ("loss", "Loss"), ("comfirm", "Comfirm")],
    ...
    def check_line(self):
        for line in self.sudo():
            if line.asset_id:
                if line.inventory_result == "profit":
                    raise UserError(_("The Asset already exists and can not profit"))
            else:
                if line.inventory_result == "comfirm":
